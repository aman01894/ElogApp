 ...
 * @author aminography
 */
class ChooseVideoHelper private constructor(
    ...
    private fun createOptionsList(): List<Map<String, Any>> {
        return if (!filePath.isNullOrBlank() || alwaysShowRemoveOption == true) {
            mutableListOf<Map<String, Any>>(
                ...
                mutableMapOf(
                    KEY_TITLE to activity.getString(R.string.gallery_photo),
                    KEY_ICON to R.drawable.ic_photo_black_24dp
                ),
                mutableMapOf(
                    KEY_TITLE to activity.getString(R.string.gallery_video),
                    KEY_ICON to R.drawable.ic_photo_black_24dp
                )/*,
                mutableMapOf(
...
//                    putExtra(MediaStore.EXTRA_SIZE_LIMIT, CAMERA_MAX_FILE_SIZE_BYTE)
    ...
    abstract class BaseRequestBuilder<T> internal constructor(
        ...
        fun build(callback: ChoosePhotoCallback<T>): ChooseVideoHelper {
            return when (which) {
                WhichSource.ACTIVITY -> ChooseVideoHelper(
                    activity!!,
                    null,
                    which,
                    outputType,
                ...
                WhichSource.FRAGMENT -> ChooseVideoHelper(
