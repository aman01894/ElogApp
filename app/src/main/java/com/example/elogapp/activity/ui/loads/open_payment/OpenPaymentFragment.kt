...
import com.example.elogapp.databinding.FragmentOpenPaymentBinding
...
class OpenPaymentFragment : Fragment(), KodeinAware, LoadListener {

    override val kodein by kodein()
    private val factory: OpenPaymentViewModelFactory by instance()
    private lateinit var rAdapter: OpenPaymentAdapter
    private lateinit var openPaymentViewModel: OpenPaymentViewModel
    private lateinit var binding: FragmentOpenPaymentBinding
    ...
    override fun onCreateView(
        ...
        binding =
            DataBindingUtil.inflate(inflater, R.layout.fragment_open_payment, container, false);
        val view = binding.root
        openPaymentViewModel = ViewModelProvider(this, factory)
            .get(OpenPaymentViewModel::class.java)
        openPaymentViewModel.loadListener = this

        dialog = activity?.let { MaterialDialog(it) }!!

//        fromOpenPayment = arguments?.getString("From").equals("OpenPayment", true)

        if(arguments?.getString("From") == "OpenPayment"){
            fromOpenPayment = true
        }else if(arguments?.getString("From") == "HistoryPayment"){
            fromOpenPayment = false
        }
    ...
    private fun callAiiForData(status: Int) {
        openPaymentViewModel.getLoadData(status)
    }
    ...
    override fun onSuccess(loadDisRes: LoadDispatchResponse) {
        ...
        for (list in loadDisRes.data) {
                ...
                if (list.status == AppConstants.ACCEPT &&
                    list.paymentStatus == AppConstants.OPEN_PAYMENT ) { //Open Payment
...
//
//                binding.relLayout.visibility = View.GONE
//            }else if (list.status == AppConstants.CLOSE &&
